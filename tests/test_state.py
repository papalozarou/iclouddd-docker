    ...
    def test_read_json_quarantines_malformed_json(self) -> None:
        with tempfile.TemporaryDirectory() as TMPDIR:
            ...
            self.assertTrue(
                any(
                    CALL.args[1] == "error"
                    and "Corrupt JSON state ignored" in CALL.args[2]
    ...
    def test_read_json_returns_empty_dict_when_open_fails(self) -> None:
        with tempfile.TemporaryDirectory() as TMPDIR:
            ...
            self.assertTrue(
                any(
                    CALL.args[1] == "error"
                    and "State read failed" in CALL.args[2]
                    for CALL in LOG_LINE.call_args_list
                )
            )
    ...
    def test_write_json_warns_when_open_fails(self) -> None:
        with tempfile.TemporaryDirectory() as TMPDIR:
            ...
            self.assertTrue(
                any(
                    CALL.args[1] == "error"
                    and "State write failed" in CALL.args[2]
                    for CALL in LOG_LINE.call_args_list
                )
            )
    ...
    def test_write_json_warns_when_temporary_cleanup_fails(self) -> None:
        with tempfile.TemporaryDirectory() as TMPDIR:
            ...
            self.assertTrue(
                any(
                    CALL.args[1] == "error"
                    and "Temporary state cleanup failed" in CALL.args[2]
                    for CALL in LOG_LINE.call_args_list
                )
            )
