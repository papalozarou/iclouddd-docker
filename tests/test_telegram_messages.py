...
class TestTelegramMessages(unittest.TestCase):
    ...
    def test_build_authentication_complete_message_preserves_plain_text_symbols(self) -> None:
        MESSAGE = build_authentication_complete_message(
            "alice_[ops]@example.com",
            "Accepted *plain* text _as-is_.",
        )
        self.assertIn("alice_[ops]@example.com", MESSAGE)
        self.assertIn("Accepted *plain* text _as-is_.", MESSAGE)
